...
def pbp_sql(years, events = None, teammates = True, opposition = True, engine = engine):
    ...
    if events == None:
        ...
        events = ['FAC', 'HIT', 'SHOT', 'STOP', 'MISS', 'GIVE',
                  'BLOCK', 'GOAL', 'TAKE', 'PENL', 'DELPEN',
                  'CHL', 'CHANGE']
...
def stats_sql(years, level = 'game', strengths = ['5v5'], players = [], teammates = False, opposition = False, score = False, engine = engine):
    ...
    group_cols = ['season', 'session', 'player', 'player_id', 'position', 'team', 'strength_state'] 
    ...
    if level == 'game' or level == 'period':
        ...
        group_cols[(group_cols.index('team') + 1):(group_cols.index('team') + 1)] = ['opp_team']
    ...
    for year in years:
        ...
        order_cols = ['season', 'session', 'game_date', 'game_id', 'team', 'opp_team',
                     'strength_state', 'score_state', 'game_period', 'toi']
        
        order_bools = ['ASC', 'ASC', 'ASC', 'ASC', 'ASC', 'ASC', 'ASC', 'ASC', 'ASC', 'DESC']
        
        order_sql = [f'{col} {order_bool}' for col, order_bool in zip(order_cols, order_bools) if col in group_cols or col in stats_cols]
...
def lines_sql(years, line_type, strengths = ['5v5'], player = False, level = 'game', teammates = False, opposition = False, score = False, engine = engine):
    ...
    if line_type == 'goalie' or line_type == 'g':
        ...
        group_cols = ['season', 'session', f'own_{line_type}', f'own_{line_type}_id', 'team'] 
    ...
    if teammates != False:
        ...
        if line_type == 'forwards' or line_type == 'f' or teammates == 'defense' or teammates == 'd':
            
            mates_cols = d_cols
        
        if line_type == 'defense' or line_type == 'd' or teammates == 'forwards' or teammates == 'f':
            
            mates_cols = f_cols
            
        if line_type == 'goalie' or line_type == 'g' or teammates == 'goalie' or teammates == 'g':
